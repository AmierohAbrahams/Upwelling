...
CC_dur <- read_csv("/home/amieroh/Documents/Masters_2019/Upwelling/global_EBUS/event_Dur/CC_Duration.csv")
BC_bathy <- read_csv("Data/bathy_EBUS/bathy/BC_bathy.csv")
CC_bathy <- read_csv("Data/bathy_EBUS/bathy/CC_bathy.csv")
...
BC.layers = list(
  ...
  coord_fixed(ratio = 1, xlim = c(10.5, 39.5), ylim = c(-39.5, -25.5), expand = TRUE),
...
CC.layers = list(
  geom_polygon(data = shore, aes(x = X, y = Y, group = PID),
               colour = "#272822", fill = "#272822", size = 0.2),
  coord_fixed(ratio = 1, xlim = c(330, 350), ylim = c(20, 40), expand = TRUE),
  scale_x_continuous(expand = c(0, 0), labels = scales::unit_format(unit = "°E", sep = "")),
  scale_y_continuous(expand = c(0, 0), labels = scales::unit_format(unit = "°S", sep = "")),
  labs(title = NULL, x = NULL, y = NULL)
)
...
AC.fig09 <- gv.plot(CC_dur, CC.layers, CC_bathy, "CC")
...
AC.fig09 <- gv.plot(BC_dur, BC.layers, BC_bathy, "BC")
```
